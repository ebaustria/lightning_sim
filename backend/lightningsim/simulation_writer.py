...
def axi_io_json_obj(
    address: int, axi_interface_io: AxiInterfaceIo, mod_data_struct: ModuleDataStruct
) -> Dict[str, Any]:
    return {
        "address": address,
...
def axi_json_arr(
    axi_io: Dict[AxiInterface, AxiInterfaceIo], mod_data_struct: ModuleDataStruct
) -> List[Dict[str, Any]]:
    return [
        axi_io_json_obj(key.address, value, mod_data_struct)
        for key, value in axi_io.items()
    ]
...
def write_simulation(
    json_data: Dict[str, Any], cu_num: int | None, data_size: int | None
):
